...
def getLocationConfirmMessage(title, latitude, longitude):
    data = {'title': title, 'latitude': latitude, 'longtitude': longitude, 
            'action': 'get_near'}
    message = {
            "type": "template",
            ...
            "template": {
                "type": "confirm",
                "text": f"請確認是否搜尋 {title} 附近地點?",
                "actions": [                 #  可搜尋Line的Action objects文件
                    {
                        "type": "postback",
                        "label": "是",
...
def getTodayCovid19Message():
    response = requests.get("https://covid-19.nchc.org.tw/api/covid19?CK=covid-19@nchc.org.tw&querydata=3001&limited=BGD", headers=HEADER)
